...
/**
 * Legacy quiz error boundary — catches errors on the public
 * /quiz/[quizSlug] page (the legacy un-scoped quiz URL).
 *
...
export default function LegacyQuizError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[LegacyQuizError]", error)
  }, [error])
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4 text-center">
          <p className="text-sm text-muted-foreground">
            We couldn&apos;t load this quiz. Try reloading the page.
          </p>
          {error?.message && (
