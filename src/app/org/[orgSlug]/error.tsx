...
/**
 * Organization landing error boundary — catches errors on the public
 * /org/[orgSlug] page (org profile + event list).
 *
...
export default function OrgLandingError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[OrgLandingError]", error)
  }, [error])
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-slate-950">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl">Organization error</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 text-center">
          <p className="text-sm text-muted-foreground">
            We couldn&apos;t load this organization. Try reloading the page.
          </p>
          {error?.message && (
