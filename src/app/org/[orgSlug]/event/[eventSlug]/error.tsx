...
/**
 * Event landing error boundary — catches errors on the public
 * /org/[orgSlug]/event/[eventSlug] page (event details + quiz start CTA).
 *
 * Mirrors the dashboard error pattern (AlertTriangle + reset).
 * Uses the emerald/teal palette for the action button.
 */
...
export default function EventLandingError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[EventLandingError]", error)
  }, [error])
